...
export function GET() {
  const lines: string[] = [
    ...
    `> ${CANONICAL_SENTENCE}`,
    "",
    `${PERSON_SENTENCE} The app is a desktop client for Mac and Windows that wraps the official WhatsApp Web, LinkedIn, and Gmail (no unofficial APIs, so the user's number and accounts stay safe) and puts an AI agent beside every chat. The agent runs on the user's own model account (Claude, ChatGPT, Gemini, or any OpenAI-compatible endpoint), drafts into the composer, and never sends anything itself. Cereal Milk grew out of an engineering studio that shipped 62 projects for 68 venture funds and firms.`,
    "",
