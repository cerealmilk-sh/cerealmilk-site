...
export function GET() {
  const lines: string[] = [
    ...
    "## Docs",
    "",
    `- [80x Docs](${SITE_URL}/docs/): open documentation on agentic engineering for venture-capital funds, playbooks (qualification, pipeline hygiene, CRM migration, LP fundraising ops), and field notes, published by the team behind 80x. Has its own index at ${SITE_URL}/docs/llms.txt`,
    "",
    "## Optional",
    "",
    `- [Sitemap](${SITE_URL}/sitemap.xml): all URLs with last-modified dates`,
    `- [GitHub](${GITHUB_ORG_URL}): 80x's open-source repositories`,
    "",
  ];
