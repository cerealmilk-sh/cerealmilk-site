...
// The public download page, the landing spot for every "Get 80x" on the
...
// platform and auto-starts the right installer: /download/80x.dmg on Mac,
// /download/80x.exe on Windows, a manual picker anywhere else. Both file
// routes first-party-log the hit, then 307 to the current release on
// 80x-org/product-releases, so this page never goes stale. The app is free
      ...
      <JsonLd
        data={graph(
          breadcrumbNode("/download", [
            { name: "Download 80x", path: "/download" },
          ])
        )}
      />
