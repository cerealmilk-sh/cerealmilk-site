How Cereal Milk keeps your number, your accounts, and your conversations safe: no unofficial APIs, on-screen reads only, you always hit send, and a server-enforced privacy gate on CRM sync. This page states plainly what the app does, what it never does, and where the honest limits are.
...
Cereal Milk does none of that. Under the hood it is the official WhatsApp Web, LinkedIn, and Gmail running inside Apple's WebKit, the same engine as Safari, pinned to a desktop Safari identity. There is no Baileys, no whatsapp-web.js, no LinkedIn Voyager scraping, and no WhatsApp Business API. To the services you use, Cereal Milk is a browser, because under the hood it is one.
...
- CRM writes go through one audited path, the same contract the Cereal Milk browser extension uses, with the privacy gate applied to both.
- Your conversations live where they always did: on your phone and with the services themselves. Cereal Milk stores what you chose to sync, and nothing else.

## Distribution you can verify

Cereal Milk ships as a DMG with Sparkle auto-update over an EdDSA-signed feed, from a public releases repository. [Download it](/download) and the install page walks every step, including the one-time macOS approval; not the Mac App Store, because Apple rejects apps that wrap another company's web service. Prefer to be set up in person? [Book a demo](/demo) and the founder does it with you.
