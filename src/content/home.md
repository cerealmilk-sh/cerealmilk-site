...
Your best deals leave no record. Sourcing happens in DMs, diligence happens in group chats, LPs live on WhatsApp, and none of it reaches the CRM:

- The deck in the padel thread: read, replied to, never filed. Six weeks later it is forty scrolls up, and the founder has signed with someone who answered twice.
- The intro in a LinkedIn DM: the intro that converts lands where no integration sees it. To your CRM, the relationship does not exist.
- The terms agreed across three apps: half in WhatsApp, half in email, closed on a call. Someone rebuilds it from memory for the IC memo, or nobody does.
...
- The exporters own your data. Chat-export services copy your founder and LP conversations into their own cloud; the record exists, but it is theirs.

So we built the missing piece: the official clients in one native window, the CRM docked beside them, and a privacy gate enforced on the server. The fund gets the record without giving up its numbers, its data, or its partners' privacy.
...
- Command palette on ⌘K: every action in the app, fuzzy-searched. Switch, sync, snippet, export. Hit return.
- Channels on ⌘1 ⌘2 ⌘3: WhatsApp, LinkedIn, Gmail. Every session stays live in one window; nothing reloads or drops.
- Sync this chat on ⌘⇧S: the open conversation, filed to its record in one keystroke, privacy gate applied.
...
Your WhatsApp is your life, not just your deal flow. Every thread is born private, new and imported alike, until you decide otherwise. You share a whole thread, or the one line that matters; the rest never moves. The privacy gate is enforced on the server: a private message is refused before a note is ever created, so private content cannot sync by accident, by cron, or by bulk action.

## The rest of the kit

Each of these exists because a fund asked for it in the field:

- Native notifications: three channels, one place that pings you, so a founder's reply never waits behind a forgotten tab.
- Voice and video calls: founders call on WhatsApp; mic and camera pass straight through, so diligence calls happen where the thread lives.
- Relationship insights: who leads, tone and warmth, and how much of the thread reached the record. Know before the Monday pipeline review asks.
- Matching that never guesses: an exact phone, email, or profile links in one click; fuzzy names are only ever suggested, so a founder's messages never land on the wrong company.
- Auto-sync shared threads: flip the live deal to Shared and every new message files itself, still behind the gate.
- Attio or Affinity: the two CRMs venture actually runs, chosen per workspace; the deepest inline record editing runs on Attio.
- One-click export: the open chat as a paste-ready deal note for the IC memo, Markdown or plain text, or the contact card.
...
- **Every relationship remembered.** The whole history of who you know, searchable in one place, on your machine.
- **Nothing dropped.** You see who is waiting on you before they chase you. Follow-ups happen while they still matter.
- **You reply first, so you close.** Deals go to whoever answers. With the context one keystroke away, that is you.
...
Only the chat you have open, and only when you ask. It never enumerates your chats or scrolls history in the background. Everything stays private until you share it, and the sharing gate is enforced server-side.
...
Both. Pick your CRM of record per workspace, with the same selective sync and the same server-enforced privacy gate against either. The deepest record integration today, inline reads and edits of stage, owner, and next steps, runs on Attio.
