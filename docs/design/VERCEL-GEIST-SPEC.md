...
The landing site themes through `--bc-*` indirection variables mapped to
...
|---|---|---|
| `--bc-bg` | `#000000` | page background |
| `--bc-bg-2` | `#0a0a0a` | raised background (code windows, wells) |
| `--bc-panel` | `#0a0a0a` | surface |
| `--bc-panel-2` | `#111111` | surface hover / inset |
| `--bc-panel-3` | `#1a1a1a` | strongest surface |
| `--bc-panel-hover` | `#111111` | hover fill |
| `--bc-edge` | `#1f1f1f` | structural hairlines (section/grid) |
| `--bc-edge-2` | `#333333` | interactive borders (inputs, secondary buttons, code windows) |
| `--bc-ink` | `#ededed` | primary text |
| `--bc-ink-dim` | `#a1a1a1` | secondary text (ledes, body) |
| `--bc-ink-faint` | `#6f6f6f` | tertiary (labels, captions) |
| `--bc-ink-ghost` | `#3f3f3f` | disabled / line numbers / crosshairs |
| `--bc-accent` | `#ffffff` | primary action (white pill) |
| `--bc-accent-bright` | `#ffffff` |, |
| `--bc-accent-dim` | `#cccccc` | pressed pill |
| `--bc-accent-deep` | `#1a1a1a` | tinted fill |
| `--bc-accent-ink` | `#000000` | text on primary pill |
| `--bc-blue` (NEW) | `#52a8ff` | links, active nav/sidebar states |
| `--bc-blue-dim` (NEW) | `#0070f3` | blue fills / focus ring base |
| `--bc-danger` | `#e5484d` | errors |
...
var(--bc-blue)` / `--color-blue-dim: var(--bc-blue-dim)` to the `@theme` block
...
- **Menu contents** (every current nav destination stays reachable):
  - **Services ⌄** → col "Services": the 3 service pages + "All services";
    col "Work": up to 5 case studies + "All work"; col left spare or a third
    grouping if natural. Long case-study titles may wrap, keep leading tight.
  - **Tools ⌄** → col "Tools": Sentry (/tools/sentry), Backchannel (/app);
    col "Open source": valentine ↗, attio-cli ↗, Attio Workflows Handbook ↗,
    awesome-attio ↗; plus "All tools" link to /tools.
  - **Resources ⌄** → col "Learn": Docs (/docs), Writing (/writing), Field
    Notes (/newsletter); col "Company": About (/about), Contact (/contact),
    Book a call (/book).
...
2. **`@theme` tokens resolve at `:root`**, the `.studio` class re-points
   `--bc-*` AND re-declares the `--color-*` mappings (existing pattern at the
