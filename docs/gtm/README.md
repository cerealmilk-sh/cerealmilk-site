...
and the `/api/{cron/drip,lifecycle}` routes. Use it to review the
...
```
   ...
   ───────►  "Request access" (waitlist)  ─────────────┐               │
                         └─────────────────────────────│───────────────┘
                                          ...
                                          └───────────┬───────────┘
                                                      ▼
                       NURTURE drip (N1–N6, daily cron)  ── converts? ──► /preorder
                       +2  pain                                            (founding seat)
                       +4  "will my number get banned?"                         │
                       +7  privacy / discretion                                 │ purchase
                       +10 a day in the cockpit                                 ▼
                       +14 the offer (skip the line)            POST /api/lifecycle
...
- **Conversion event:** `/preorder` → payment → `POST /api/lifecycle` `{"type":"converted"}` (pay the
...
| `type` | Meaning | Effect |
|---|---|---|
| `converted` | purchased | stop nurture, start activation |
...
3. **Purchase signal**: nothing fires `converted` automatically: when a
   founding seat is sold, `POST /api/lifecycle` with `{"type":"converted",
   "email":…}` and the `x-lifecycle-secret` header (`LIFECYCLE_INGEST_SECRET`).
...
- **No automated purchase signal.** No payment webhook is wired: `converted`
  must be POSTed to `/api/lifecycle` when a founding seat is sold (manually or
  from the payment flow). Until then conversions don't stop the nurture drip.
- **Activation depends on not-yet-emitted signals.** Until something fires
  `converted` and the Mac app posts usage signals, the activation track
